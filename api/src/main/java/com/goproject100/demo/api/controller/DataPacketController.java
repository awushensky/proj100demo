...
public class DataPacketController {
    ...
    public @ResponseBody DataPacketResponse postNewData(final @RequestBody DataPacket dataPacket) {
        ...
        try {
            ...
            return DataPacketResponse.newFailureResponse(e);
        }
    ...
    public @ResponseBody DataPacketResponse getRecentPackets(final @PathVariable("number") int numberToRetrieve,
                                                           final @PathVariable("type") UserType type) {
        LOG.debug("GET request for the last {} data packets for type {}", numberToRetrieve, type);

        final List<DataPacket> response;
        try {
            response = dataPacketService.getRecentDataPackets(numberToRetrieve, type);
        } catch (Exception e) {
            return DataPacketResponse.newFailureResponse(e);
        }
    ...
    public @ResponseBody
    DataPacketResponse registerNewEndpoint(final @RequestBody String url,
                                           final @PathVariable("type") UserType type) {
